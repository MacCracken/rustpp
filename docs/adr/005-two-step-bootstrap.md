...
**Status**: Accepted
**Date**: 2026-04-05
**Context**: Heap layout changes break the bootstrap chain because the committed cc2 binary uses old offsets.
...
1. Old cc2 compiles new source → cc3 (has new layout, may differ from cc2)
2. cc3 compiles same source → cc4 (must equal cc3)
3. Verify cc3 == cc4 (byte-identical self-hosting)
4. Copy cc3 to build/cc2

## Rationale

- The old cc2 can compile the new source because it only uses ITS OWN offsets during compilation
...
- The committed build/cc2 must always be the SECOND-generation binary
