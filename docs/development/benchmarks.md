...
| v5.6.25 | 542,928 | +17,584 | **aarch64 push/pop-cancel completion.** x86 cc5 unchanged (this slot only touches aarch64 backend). Native aarch64 cc5 shrank **517,376 → 497,008 B (−20,368 B / −3.94%)** — bigger than v5.6.11's aarch64 combine-shuttle win (−17,672 B). Scope retargeted from originally-planned "aarch64 fused ops" after bytescan under v5.6.24 default-on regalloc found **0** `mul+add` / **0** `lsr+and-imm` adjacent pairs across all math-heavy tcyr targets. Cyrius stack-machine IR keeps a push between every sub-expression result and its consumer, so fused-op windows never open without a separate IR-level push-elision pass (pinned future slot). Bytescan DID find **2,569 adjacent `push x0; pop x0` pairs** in native aarch64 cc5 — v5.6.9's cancel mechanism had a latent gap: `EPOPR` did the adjacency check but `EPOPARG(S, n)` — which emits the same `ldr xN, [sp], #16` for N=0..7 — never did, so every 1-arg call site (EPUSHR; EPOPARG(S, 0)) paid 8 bytes. 13-LOC fix in `src/backend/aarch64/emit.cyr::EPOPARG` mirroring EPOPR's `GCP == _last_push_cp` rewind, gated on `n == 0`. Residual push/pop pairs in new binary: **0**. check.sh 23/23, 3-step fixpoint clean at IR=0 and IR=3 (x86 side), Pi broad-scope exit42 OK. |
| v5.6.24 | 542,928 | +17,584 | **Default-on regalloc + SysV ECALLPOPS callee-saved-clobber fix.** Two pieces in one slot (sandhi-reported v5.6.25 live-across-calls consolidated — same root). (1) `src/backend/x86/emit.cyr::ECALLPOPS` SysV n>6 path used r12/r13/r14 as scratch for the stack-arg shuttle. r12-r14 are CALLEE-SAVED in SysV; under v5.6.20+ regalloc the picker pinned caller's locals to those callee-saved regs, so 8+ arg calls silently corrupted the locals. Surfaced as `flags.tcyr test_str_short → test_defaults` (AUTO_CAP=118: test_str_short's `_argv(...)` 9-arg call clobbered r12 holding `fs` in `test_defaults`'s frame) AND sandhi M2 `sandhi_http_response_parse`'s `var headers` zeroed by a downstream call. Fix: rewrite SysV ECALLPOPS to use only **r10 (caller-saved)** as scratch via direct `[rsp+offset]` MOV — no pops, single shift loop, end with `add rsp, 48`. (2) Flipped `_ra_auto_cap` default from -1 (disabled) to "uncapped". Every x86 fn without inline asm now auto-regalloc'd. cc5 522,624 → 542,928 B (+20,304 B for the save/restore overhead at every fn × 5 callee-saved regs); aarch64 cross 400,872 → 419,776 B; win cross 516,200 → 537,896 B. Self-host compile time effectively unchanged (~0.46s) — cc5's hot path is parse/codegen with small fns, not pinned-local-heavy compute, so the picker's wins balance against prologue overhead. Real gains are in downstream consumers (patra/sandhi/ark) where 5+ pinned locals dominate hot paths. Both fixpoints clean (IR=0 b==c, IR=3 b==c at 542,928 B). check.sh 23/23, all 84 .tcyr PASS, `regression-inline-asm-discard.sh` PASS at default + AUTO_CAP=99999. Sandhi-pinned `fdlopen_init_full` + `lib/tls.cyr` HTTPS infinite-loop deferred (no slot yet — orthogonal investigations). Cascade -1 (closeout v5.6.36 → v5.6.35). |
| v5.6.23 | 522,624 | −2,720 | **Misdiagnosis correction — inline-asm + regalloc stack-frame layout fix.** v5.6.22's "v5.5.21 array-alignment regression" framing was wrong. Real root cause: regalloc's prologue saves rbx/r12-r15 to `[rbp-8]..[rbp-40]`; local-var disps are computed as `-(idx + 1 + _cur_fn_regalloc) * 8`, shifting every spilled local/param 40 B further from rbp. Inline asm blocks hardcode disps like `mov rdi, [rbp-0x08]` for param 1 (standard cyrius prologue) — under auto-enable that now dereferences the saved RBX value and SIGSEGVs. Alignment probe confirmed the array VA was 16-aligned; crash is inside the asm on layout mismatch. Fix: `parse_fn.cyr` body-scan lookahead for token 48 (`asm`) before the auto-enable / opt-in `#regalloc` decision. Auto-enable silently skips; opt-in `#regalloc` on an asm fn warns and skips. Stdlib callers protected (`lib/fnptr.cyr`, `lib/thread.cyr`, `lib/atomic.cyr`, `lib/hashmap_fast.cyr`, `lib/u128.cyr`, `lib/thread_local.cyr`, `lib/fdlopen.cyr`, `lib/args_macos.cyr`). Default-on flip attempt surfaced a **SECOND picker bug** — bisection `CYRIUS_REGALLOC_AUTO_CAP=117/118` → 118 = `test_str_short` (5 vars, 2 calls) regalloc-enabling corrupts the NEXT fn `test_defaults`'s 5-arg `flags_add_int(fs, 0, "count", 7, "")` call (default_val 7 → 0). Different shape from v5.6.22 loop-back time-share; independent of asm-skip (present) and NOP-fill (attempted and reverted within v5.6.23). Pinned v5.6.24. Consumer-surfaced live-across-calls (patra-side boxing workaround, 2026-04-24) pinned v5.6.25. Both fixpoints clean (IR=0 b==c, IR=3 b==c at 522,624 B). check.sh 23/23. `regression-inline-asm-discard.sh` PASS under `CYRIUS_REGALLOC_AUTO_CAP=99999`. cc5 grew +1,408 B (521,216 → 522,624). Regression-stub pin labels cascade +2 (v5.6.30/31/32 → v5.6.32/33/34); roadmap closeout v5.6.34 → v5.6.36. |
| v5.6.22 | 521,216 | −4,128 | **Phase O4c (partial): picker correctness fix + auto-enable infra (disabled by default).** v5.6.20's time-sliced register reuse silently broke loops — when picker reassigned an expired interval's register, JMP_BACK to a position INSIDE the earlier interval's CP range read stale register state. Surfaced via v5.6.22 auto-enable bisection on `PP_ALREADY_INCLUDED`. Fix: extend `interval.last_cp = ra_end` so picker can no longer time-share. Effectively reverts to single-register-per-local-for-whole-fn (greedy-equivalent). `CYRIUS_REGALLOC_AUTO_CAP=N` env knob shipped (default -1 = disabled, N>0 opts in). Default-on **deferred to v5.6.23** under a "v5.5.21 array-alignment regression" framing (v5.6.23 traced the actual cause to inline-asm + regalloc stack-frame layout — not alignment). Bisection methodology (v5.6.17 saved pattern) worked perfectly: AUTO_CAP=303→304 isolated PP_ALREADY_INCLUDED; PICKER_CAP=2→3 isolated time-share REUSE. Both fixpoints clean (IR=0 b==c, IR=3 b==c at 521,216 B). check.sh 23/23. cc5 grew +712 B for cap knob + time-share fix. Patra 1.6.0 verified folding cleanly. |
| v5.6.21 | 520,504 | −4,840 | **Codegen bug fix — bare-truthy `if (r)` after fn-call (patra-blocking).** A v5.6.x regression surfaced by user testing. `var r = helper(); if (r) {...}` took FALSE branch when r==1. Root cause: v5.6.8's `_flags_reflect_rax` (Phase O2 cat 2) global tracker was NOT reset by 4 emit fns (EFLLOAD, ECALLFIX, ECALLTO, ESYSCALL); ECONDCMP's stale-flag skip then branched on flags from inside the callee. Fix: 4 `_flags_reflect_rax = 0;` resets in `src/backend/x86/emit.cyr` (+48 B). New regression gate 4r in check.sh — `tests/regression-truthy-after-fncall.sh` (self-contained, no stdlib deps; 2 shapes: var-intermediate + direct `if (fn_call())`). Both fixpoints clean (IR=0 b==c, IR=3 b==c at 520,504 B). Repro `/tmp/cyrius_5.6_codegen_bug.cyr` exits 99 (was -1). Patra 1.6.0 unblocked (folds in without `!= 0` workaround across patra source). Slot count 32 → 33 (Phase O4c cascaded to v5.6.22; closeout v5.6.32 → v5.6.33). |
