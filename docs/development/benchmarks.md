...
| v5.6.22 | 521,216 | −4,128 | **Phase O4c (partial): picker correctness fix + auto-enable infra (disabled by default).** v5.6.20's time-sliced register reuse silently broke loops — when picker reassigned an expired interval's register, JMP_BACK to a position INSIDE the earlier interval's CP range read stale register state. Surfaced via v5.6.22 auto-enable bisection on `PP_ALREADY_INCLUDED`. Fix: extend `interval.last_cp = ra_end` so picker can no longer time-share. Effectively reverts to single-register-per-local-for-whole-fn (greedy-equivalent). `CYRIUS_REGALLOC_AUTO_CAP=N` env knob shipped (default -1 = disabled, N>0 opts in). Default-on **deferred to v5.6.23** — v5.5.21 array-alignment fix interacts with auto-enable's code growth in a way the per-array padding misses; `tests/regression-inline-asm-discard.sh` SIGSEGVs on SSE m128 ops with auto-enable. Bisection methodology (v5.6.17 saved pattern) worked perfectly: AUTO_CAP=303→304 isolated PP_ALREADY_INCLUDED; PICKER_CAP=2→3 isolated time-share REUSE. Both fixpoints clean (IR=0 b==c, IR=3 b==c at 521,216 B). check.sh 23/23. cc5 grew +712 B for cap knob + time-share fix. Patra 1.6.0 verified folding cleanly. |
