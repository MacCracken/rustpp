...
| 4 | ~~`break` in deeply nested while/if~~ | agnostik, json.cyr | **Resolved 3.3.9** | Break patches expanded from single slot to 16-entry array at `0x20000`. Multiple breaks per loop now patched correctly. While, for-in, for-each, C-style for all updated. |
