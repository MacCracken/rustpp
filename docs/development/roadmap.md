...
> **v1.10.2.** 205KB self-hosting compiler, both architectures.
...
|---|-------|----------|--------|
| 12 | **`#derive(Serialize)` runtime segfault** | P2 | The `_to_json` function generated by derive crashes at runtime. The field name truncation (bug #8) is fixed, but the generated code has a separate memory access issue. Workaround: implement `_to_json` manually using `json_pair_new` + `json_build`. |
