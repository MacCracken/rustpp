...
> **v5.5.29.** cc5 compiler (488,864 B x86_64), x86_64 + aarch64
...
> v5.5.30 orchestration completion pinned with concrete diagnostic
> starting points), v5.5.31 TLS via `arch_prctl`, v5.5.32 atomics,
...
| **v5.5.30** | `lib/fdlopen.cyr` orchestration — completion | Finishes what v5.5.29 started. Concrete diagnostic starting points (from the v5.5.29 investigation log, pinned here so v5.5.30 doesn't start from a blank page): (1) **Dump AT_PHDR bytes** — compare `helper_info + 16` against file offset 0x40..0x350 of `~/.cyrius/dlopen-helper`. Mismatch means the copy pass missed an offset. (2) **File-backed mmap of helper PT_LOAD** — switch from anon+memcpy to `mmap(base+p_vaddr, p_memsz, prot, MAP_PRIVATE|MAP_FIXED, fd, p_offset & ~4095)` per segment. Cosmopolitan uses file-backed; may be load-bearing for ld.so's `/proc/self/maps` introspection. (3) **strace diff** — `strace -f ./dlopen-helper ADDR ADDR` vs probe-with-strace. First diverging syscall pinpoints the failure. (4) **Cosmopolitan cosmo_dlopen.c cross-ref** — walk the pre-jump setup (locale init, TLS base setup, signal mask save, stack canary) for steps v5.5.29 skips. **Acceptance:** `fdlopen_init_full` returns 0 on glibc host; `fncall0(fdlopen_dlopen(state))` loads libresolv; `fncall4(fdlopen_getaddrinfo(state), ...)` succeeds; check.sh gate 4j gains a "full path" sub-test; self-host byte-identical. |
| **v5.5.31** | TLS via `arch_prctl(ARCH_SET_FS)` | `%fs:`-relative addressing for thread-local globals. Queued from v5.4.10 thread work — majra's `_aaw_result_state` global needs TLS to be thread-safe. Must land after v5.5.30 because fdlopen's ld.so-entry jump clobbers %fs; documenting the call-order constraint lives with this patch. |
