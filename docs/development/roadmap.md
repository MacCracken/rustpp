...
> - **v5.7.9**: ✅ shipped 2026-04-26 — silent fn-name collision investigation. cc5 emits `warning: ... duplicate fn '<name>' (last definition wins)` at registration. Stdlib audit found one genuine collision (json_build): patra renamed to `patra_json_build` + minor bumped 1.8.3 → 1.9.0. Internal `EADDIMM_X1` collision in cc5 surfaced + dead imm8 form deleted. cc5 +88 B. check.sh 30/30.
> - **v5.7.10**: ✅ shipped 2026-04-26 — `input_buf` 512KB → 1MB heap-map reshuffle. Load-bearing for the ecosystem (hisab at 96% of cap, censoring upstream). 95 distinct heap-region addresses shifted +0x100000 (not naive +0x80000 — would have collided with 6-digit squatters at 0x104000-0x14A000). 96 bare-hex comment refs swept; 4 boundary-comment over-shifts hand-corrected. brk unchanged at 0x348C000. cc5 unchanged at 709,776 B (heap-only). New regression `tests/regression-input-1mb.sh` (gate 4t).
> - **v5.7.11**: ✅ shipped 2026-04-27 — `main_cx.cyr` drift fix + CI gate (smaller-slot per user direction "correctness over new features"). 4 missing pieces (ir.cyr include + 8 globals) + 2 dead PF64 cx-stubs deleted + brk 5.5 MB → 39 MB. New `tests/regression-cx-build.sh` (gate 4u). Bytecode semantic correctness cascaded to v5.7.12.
> - **v5.7.12**: ✅ shipped 2026-04-27 — cyrius-x bytecode path-B (`_TARGET_CX == 0` guards). 7 sites in parse_*.cyr guarded; cc5_cx output is now clean CYX bytecode (zero x86 instruction-byte pollution). Path A (named-op refactor across 3 backends) pinned long-term in roadmap. cc5 +536 B (710,312 B). New `tests/regression-cx-roundtrip.sh` (gate 4v). Pre-existing cx codegen literal-arg propagation bug pinned for follow-up patch slot.
> - **v5.7.13**: RISC-V rv64 port (inherits optimized compiler + post-fold stdlib shape + bumped fixup table + complete cyrius-ts frontend + collision-resolution rule + 1MB input_buf + cx-build-clean + cx-path-B). Slid across 2026-04-24/27 as sandhi fold/cyrius-ts/fixup-cap/JSX/JSX-AST/JSX-inner-expr/cyrius-check-and-deps/collision-investigation/input-buf/cx-drift/cx-path-B took priority slots in turn.
