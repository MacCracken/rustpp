...
> **v5.6.7.** cc5 compiler (526,272 B x86_64), x86_64 + aarch64
> cross + Windows PE cross + macOS aarch64 cross. IR + CFG.
> Self-hosts byte-identically on Linux x86_64, Linux aarch64 (Pi
> 4), Windows 11, and macOS arm64. **v5.6.7 opens Phase O2 peephole
> with strength reduction** (`x * 2^n → shl`/`lsl`): cc5 shrank
> 528,144 → 526,272 B (−1,872 B / −0.35 %) net. First optimizer
> patch that modifies generated bytes; established the 3-step
> fixpoint verification pattern for all subsequent O-arc patches.
> **Scope note:** v5.6.7 shipped 1 of the 5 peephole categories
> from the roadmap; remaining 4 (flag-reuse, move-elim, LEA
> combining, aarch64 fused ops) await slot assignment.
>
...
> - **v5.6.7**: ✅ shipped — Phase O2 **partial** — strength
>   reduction only (`x * 2^n → shl`). Other 4 peephole categories
>   (flag-reuse, move-elim, LEA combining, aarch64 fused ops)
>   need a slot from the user.
> - **v5.6.8–v5.6.11**: compiler-optimization arc continues — O3
>   IR-driven passes, O4 linear-scan regalloc, O5 maximal-munch,
>   O6 slab allocator. Remaining O2 peephole categories may land
>   in-scope here OR as their own patches; user call.
...
### v5.6.7 — Phase O2: peephole quick wins (x86_64 + aarch64) ✅ partial (1/5)
