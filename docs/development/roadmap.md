...
| 6 | **`cyrb check` cannot validate files that use `include`** | cyrb | Medium | `cyrb check src/module.cyr` fails for any file that references functions from included libraries (e.g., `strlen`, `alloc`, `Ok`, `Err`). Only the top-level entry point (`src/main.cyr`) can be checked because it includes all dependencies. Individual module files pass the lexer but fail on unresolved identifiers. This means `cyrb check` is useless for CI per-file validation on multi-module projects. **Expected**: `cyrb check` should either follow `include` directives, accept a `--includes` flag, or have a `--syntax-only` mode that skips identifier resolution. **Workaround**: only `cyrb check` the entry point, not individual modules. Discovered during agnosys port (21 modules, 8559 lines). |
| 7 | **Identifier buffer overflow gives misleading error** | cc2 | Medium | When a program exceeds the identifier table capacity (e.g., all 20 agnosys modules + stdlib = ~865 symbols), cc2 prints `error: identifier buffer full` with no guidance. The error message does not say what the limit is, how many identifiers were used, or suggest splitting into separate compilation units. **Expected**: `error: identifier table full (865/1024) — split source into separate compilation units or reduce included modules`. Discovered during agnosys port when attempting to include all 20 modules in one compilation unit. |
| 8 | **`var beneath[16]` array size semantics undocumented** | cc2 | Low | `var buf[N]` allocates N i64 slots (N*8 bytes), not N bytes. This is not documented in the language guide or tutorial, and the syntax visually suggests byte count to C programmers. The agnosys port has multiple places where the intent was byte-count but the actual allocation is 8x larger (harmless over-allocation, but confusing). Structs passed to kernel syscalls (e.g., `sockaddr_nl`, `LandlockPathBeneathAttr`, `sock_fprog`) require precise byte-level layouts built with `store8/16/32/64`, making the `var buf[N]` semantics critical to document. **Expected**: add a note to `docs/cyrius-guide.md` clarifying `var buf[N]` allocates N*8 bytes, and recommend `var buf[N]` where N = `ceil(byte_count / 8)`. |
| 9 | **Benchmark programs segfault when `agnosys_hostname` called in tight loop** | cc2 | Medium | A benchmark program that calls `agnosys_hostname(&hbuf)` (stack-buffer API) 100K times in a while loop segfaults partway through. The function internally declares `var buf[392]` (3136 bytes of stack), calls `syscall(SYS_UNAME_NR, &buf)`, then `memcpy` to the caller's buffer. Possible causes: (1) deep stack usage from 3136-byte local in a tight loop, (2) interaction between large stack frames and cc2's stack layout, (3) stack overflow without guard page detection. The same function works fine when called once or a few times. Identical logic for `query_sysinfo` (120-byte local) works in tight loops. Threshold appears to be around stack locals > ~2KB in hot loops. **Workaround**: reduce stack buffer sizes or move large buffers to heap (`alloc`) for functions called in tight loops. Discovered during agnosys benchmark suite. |
