...
- **v5.6.22 — Phase O4c (partial): picker correctness fix + auto-enable infra (disabled by default)**: shipped two pieces, deferred default-on. (1) **Picker correctness**: v5.6.20's time-sliced register reuse silently broke loops — when an active interval expired and its register was reassigned, JMP_BACK to a position INSIDE the earlier interval's CP range read the register expecting the earlier value but found the new one. Surfaced during v5.6.22 auto-enable bisection on `PP_ALREADY_INCLUDED`'s nested while loops. Fix: extend `interval.last_cp = ra_end` for every interval — picker can no longer time-share registers (effectively reverts to single-register-per-local-for-whole-fn = greedy-equivalent). Proper time-sharing needs cross-BB liveness analysis (extend last_cp through backward edges) — pinned future slot. (2) **Auto-enable infra**: `CYRIUS_REGALLOC_AUTO_CAP=N` env knob (caps how many fns get auto-regalloc; -1 = disabled by default; N>0 opts in for first N fns), `_ra_auto_cap` + `_ra_auto_count` globals, gating in PARSE_FN_DEF, x86-only guard. **Default-on deferred to v5.6.23** because the v5.5.21 array-alignment fix interacts with auto-enable's per-fn code growth in some shape that the per-array padding misses — `tests/regression-inline-asm-discard.sh` SIGSEGVs on SSE m128 ops over `var rk[240]` when auto-enable shifts globals. Bisection methodology (v5.6.17 saved pattern) worked perfectly: AUTO_CAP=303 clean, =304 broken (PP_ALREADY_INCLUDED culprit); PICKER_CAP=2 clean, =3 broken (time-share REUSE was the trigger). Both fixpoints clean (IR=0 b==c, IR=3 b==c at 521,216 B). check.sh 23/23. cc5 grew +712 B for cap knob + time-share fix. Patra 1.6.0 verified folding cleanly.
