...
2. **Build compiler**: `cat src/main.cyr | ./build/cc5 > /tmp/cc5 && chmod +x /tmp/cc5`
3. **Make changes** to compiler or libraries
4. **Test**: `sh scripts/check.sh` (self-host + heap audit + .tcyr tests + lint)
5. **Self-host verify**: two-step bootstrap (cc5 compiles itself byte-identical)

## Rules

- Every change must pass full audit (`cyrius audit`) with 0 failures
- Self-hosting must remain byte-identical (cc5 == cc5)
