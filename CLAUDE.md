...
- **Compiler**: 488,776 B (x86_64, v5.6.15 — +1,736 B from v5.6.10 baseline of 487,040 B due to v5.6.12 IR_RAW_EMIT instrumentation + v5.6.14 IR_NOP loop-top markers + v5.6.14 IR_RAX_CLOBBER table; v5.6.15 ESETCC reorder is bytes-neutral at IR off; native aarch64 cc5 ~453,688 B at v5.6.11), aarch64 cross-compiler + native self-host byte-identical on real Pi (v5.3.15+), **`regression.tcyr` 102/102 PASS on aarch64 (v5.3.18)**, Apple Silicon Mach-O target **(self-hosts byte-identically on M-series as of v5.3.13 — 475320 B, Linux cross == Mac round 1 == Mac round 2; v5.5.11 libSystem probe, v5.5.12 compiler-driven `__DATA_CONST + __got + LC_DYLD_INFO_ONLY` bind emission, v5.5.13 first `__got` reroute — `syscall(60, code)` tail-calls `libSystem._exit`; v5.5.14 `__got` grows 1 → 6 slots — `_exit`/`_write`/`_read`/`_malloc`/`_fopen`/`_pthread_create` bound at load, `syscall(1)` → `_write` + `syscall(0)` → `_read` wired via new `FIXUP_ADRP_LDR` that scales LDR imm12 /8 for non-zero slots; v5.5.15 demotes parse.cyr's Mach-O syscall whitelist hard-error to a warning — all 4 tool binaries cross-compile to arm64 Mach-O; v5.5.16 predefines `CYRIUS_TARGET_MACOS` + `lib/alloc.cyr` → `lib/alloc_macos.cyr` (mmap) + `lib/io.cyr` flock Linux-gated; v5.5.17 adds Mach-O entry prologue (`stp x0,x1,[sp,#-16]!; mov x28, sp`) + `lib/args_macos.cyr` reads x28 via inline asm — argv works on macOS, all 4 Cyrius tool binaries do real work on `ssh ecb`. **macOS aarch64 target closed.**)**, **aarch64 Linux stdlib shakedown (v5.5.18): `lib/io.cyr` routed through per-arch `sys_*` wrappers (was Linux x86_64 syscall numbers — `file_open`=`io_setup` on aarch64, same class as v5.4.11's yukti fix that missed io.cyr); regression expanded from 2 → 5 sub-tests on `ssh pi` — alloc_ok / fs_ok / mt_ok / mutex_ok (4-thread spawn, contended futex mutex all PASS on real Pi 4). v5.5.19 inline-asm bug investigation — hit "3 attempts defer" rule on root-cause; shipped narrower repro (shape-sensitive to preceding-globals presence + discarded-result sys_write after ~120 B asm block, NOT include boundary as originally framed), `tests/regression-inline-asm-discard.sh` gate 4f in check.sh (11/11 now), workaround recipe so sigil 2.9.1 can wire AES-NI as-is. Root-cause fix pinned to v5.5.21 (not floating). v5.5.x roadmap audited: u64-hashmap slot that had a detail section but was missing from the release table got slotted at v5.5.20; inline-asm fix at v5.5.21; NSS/PAM → v5.5.22, cascade through closeout at v5.5.28. v5.5.20 shipped: `lib/hashmap.cyr` gains `map_u64_*` (SplitMix64 hash, 16 B slot with sentinel encoding — 0=empty, u64::MAX=tombstone), zero alloc on get/has/overwrite hot path verified by 30-assertion test, mabda 15× Rust cache-hit gap unblocked. v5.5.21 shipped: v5.4.15 sigil AES-NI bug FIXED — root cause was cyrius placing array globals at 8-byte-aligned addresses when SSE m128 ops (PXOR/MOVDQA/AESENC m128) require 16-byte alignment. 4-line fix in `src/backend/x86/fixup.cyr` prefix-sum pass: arrays (size > 8) now 16-aligned. v5.5.19's "preceding-globals" framing was a symptom (shifted gvars enough to flip rk's mod-16 value). Two-step bootstrap byte-identical; regression test asserts both shapes; sigil 2.9.1 can now wire AES-NI cleanly without layout-dependent workaround. v5.5.22 shipped: `cyrfmt --write` / `-w` in-place file rewrite (sankoch unblock) — idempotent short-circuit preserves mtime on already-clean files. `tests/regression-cyrfmt-write.sh` adds check.sh gate 4g (12/12 now). cc5 unchanged (program-only change). Roadmap shifted: NSS/PAM → v5.5.23, cascade through v5.5.29 closeout. v5.5.23 shipped: **scope narrowed** after a staged reproducer found the NSS/PAM pillar was actually two separate crashes. Shipped the locale half via `dynlib_bootstrap_locale(hc)` (newlocale + uselocale, sidesteps `__libc_setlocale_lock` SIGSEGV). Also bundles three consumer dep bumps (all tagged against v5.5.22): mabda 2.1.2 → 2.5.0, patra 1.1.1 → 1.5.4, sankoch 2.0.0 → 2.0.1. NSS dispatch half re-pinned as v5.5.24 with honest scope (glibc 2.34 retired `libnss_files.so.2` dlopen model, so fix now has to bootstrap libc's internal NSS state). Backstop cascaded: closeout now v5.5.30. v5.5.24 shipped: another honest 3-attempts-defer — NSS full dispatch still not tractable. Shipped the narrow piece that fell out (`dynlib_bootstrap_environ(hc)` unblocks `getenv` by initialising `__environ` / `environ` away from NULL). Investigation log at `docs/development/issues/dynlib-nss-bootstrap.md`. NSS dispatch full fix (auxv synthesis approach) re-pinned to v5.5.25 with "may never land" caveat. Closeout now v5.5.31. v5.5.27 shipped: second of the three-patch NSS real-fix arc. `lib/shadow.cyr` reads /etc/shadow via same musl pattern (rc=-1 on non-root EACCES). `lib/pam.cyr` wraps /usr/sbin/unix_chkpwd (Linux-PAM setuid-root helper) for non-root password verification — same path `pam_unix.so` itself takes from unprivileged processes. `pam_unix_authenticate(user, password)` forks helper, pipes password, returns OK/FAIL/-N infrastructure-error. Scope narrowed from inline-SHA-512-crypt plan (Drepper ~120 LoC error-prone; unix_chkpwd is ~1 ms and covers every crypt type). Also bundled: `cyrius distlib` compile-check softened fatal → info (fixes patra 1.5.5 release CI regression on consumer-bundle unresolved symbols), `cyrius bench` no-args walker now discovers `benches/*.bcyr` + `tests/bcyr/*.bcyr` like fuzz/test (takumi UX gotcha). check.sh 13 → 14. cc5 unchanged at 488,864 B. Patra pin bumped 5.5.26-1 → 5.5.27. v5.5.26 shipped: first of the three-patch NSS real-fix arc. `lib/pwd.cyr` + `lib/grp.cyr` port musl's `/etc/passwd` + `/etc/group` readers to pure cyrius — bypass glibc NSS entirely (what musl does; ~50 LoC of real logic per lookup; handles 95% of Linux deployments where `passwd: files` is default). 26-assertion test, gate 4h regression. Also bundled: reserved-keyword error diagnostic ("expected identifier, got reserved keyword 'match' (cannot be used as an identifier; rename the variable/field/fn)" instead of the misleading "got unknown"); `TOKNAME` gained 6 previously-missing keyword cases (23 else, 58 case, 59 default, 76 in, 78 shared, 79 object); `IS_KEYWORD_TOK` helper. **Also switched patra 1.5.4 → 1.5.5** — moved bundle generation from `scripts/bundle.sh` to `cyrius distlib` (one less ecosystem shell script). Size: cc5 486,552 → 488,864 B (+2,312 B = 0.48%). Known pre-existing issue: `cyrius test` auto-prepends deps before the test's own stdlib includes, breaking any test with dep → stdlib references (PROT_READ / SYS_LSEEK / SYS_MMAP undefined); affects harness only; direct `build/cc5` compile passes every test; check.sh 13/13 green. Tracked for v5.5.27+ as its own scoped work. v5.5.25 shipped: the auxv "may never land" caveat is now confirmed. Third 3-attempts-defer in the NSS arc. `programs/nss_probe.cyr` found that GLOB_DAT resolution IS already working — the v5.5.24 SIGFPE was not a GOT bug but a zero-field bug: `_rtld_global_ro` is mapped-but-zero-filled because ld.so's `_dl_start` never ran on our static binary. Populating fields cascades to more SIGSEGVs. Shipped narrow primitives (`dynlib_read_auxv` + `dynlib_auxv_get`) + `programs/nss_probe.cyr`. **Same-day external-research re-plan** (after dual agent research converged on glibc maintainers' own Weimer-2017 guidance "stop fighting `_rtld_global_ro`"): the full NSS fix is PINNED to a three-patch arc — v5.5.26 `lib/pwd.cyr` + `lib/grp.cyr` (musl-style, bypasses NSS entirely), v5.5.27 `lib/shadow.cyr` + shakti PAM via sigil crypt + /usr/sbin/unix_chkpwd fallback, v5.5.28 `lib/fdlopen.cyr` (Cosmopolitan / pfalcon foreign-dlopen pattern) for cases needing real glibc state. Closeout cascaded v5.5.31→v5.5.34. `tests/tcyr/dynlib_init.tcyr` +10 assertions (33 total). cc5 486,552 B unchanged. Vidya entry `static_dlopen_and_nss_bootstrap_limits` saved so future investigators skip the dead-end.**, **Windows PE32+ target (v5.4.2 structural, v5.4.3 `EEXIT` Win64 + IAT fixup, v5.4.4 `syscall(60)` rerouted, v5.4.5 on-hardware CI gate, v5.4.6 `#pe_import` directive, v5.4.7 `syscall(1)` → `GetStdHandle + WriteFile`, v5.4.8 PE data placement — `hello\n` runs end-to-end on Windows; v5.5.0 foundation: `build/cc5_win` cross-entry + `lib/syscalls_windows.cyr` + `lib/alloc_windows.cyr` + `CYRIUS_TARGET_WIN/LINUX` selectors; v5.5.1 bundles 5 more reroutes — `syscall(0)`→ReadFile, `syscall(2)`→CreateFileW, `syscall(3)`→CloseHandle, `syscall(8)`→SetFilePointerEx, `syscall(9)`→VirtualAlloc; v5.5.2 adds enum-constant sc_num folding so `syscall(SYS_WRITE, ...)` via the `lib/syscalls_windows.cyr` wrappers routes cleanly through the IAT instead of falling through to the 0F 05 Linux encoding; v5.5.3 flips EPOPARG + ESTOREREGPARM to Win64 arg registers (RCX/RDX/R8/R9) under `_TARGET_PE` for cyrius-to-cyrius fn calls with ≤4 args; v5.5.4 completes the call-site Win64 ABI — `ECALLPOPS` shuttles extras via r10/r11/r14/r15 for >4 args, `ECALLCLEAN` unwinds the stack-arg frame, `ESTOREPARM` dispatch boundary flips to pidx<4, `ESTORESTACKPARM` reads [rbp+16+(pidx-4)*8]; v5.5.5 fixes the `&fn` PE VA fixup in `src/backend/x86/fixup.cyr` ftype=3 (3-line PE branch); v5.5.6 adds `lib/fnptr.cyr` Win64 `fncallN` variants + env-var→predefine fix + ECALLPOPS nextra=5 extension; v5.5.7 lifts strict Win64 shadow-space compliance retroactively — every cyrius-to-cyrius call now allocates 32 B shadow, ESTORESTACKPARM shifts to [rbp+16+32+(pidx-4)*8], fnptr.cyr fncallN blocks add shadow + stack args at [rsp+32..]. Closes C-FFI-via-fnptr gap; v5.5.8 swaps main_win.cyr's heap init from SYS_BRK to SYS_MMAP (Windows has no brk); v5.5.9 gates three `/proc/self/*` Linux-ism readbacks behind `#ifdef CYRIUS_TARGET_LINUX` enabling cc5_win.exe to read stdin + compile + write PE on Windows; **v5.5.10 fixes `EWRITE_PE`: was returning WriteFile's BOOL success flag (1) instead of bytes-written. main_win.cyr's write loop did `written += w` where w=1, so olen=1536 loop iterated 1536× writing `olen + (olen-1) + ... + 1 = 1,180,416 B` = the observed output bloat. 5-byte fix (`mov rax, [rsp+0x28]` before frame unwind) mirroring EREAD_PE's existing handling. NATIVE WINDOWS SELF-HOST BYTE-IDENTICAL FIXPOINT achieved: cc5_win.exe's output matches Linux cross-build byte-for-byte (verified md5 match on exit42 and multi-fn add). Cyrius exit42 PE = 1536 B vs Rust -O stripped = 344856 B (225× smaller). check.sh 10/10.**)**, **v5.4.8 also fixes the cc5_aarch64 cross-compile `&local` x86-leak (parse.cyr now arch-dispatches the `&local` emit; yukti `core_smoke` + main CLI run exit-0 on real Pi 4)**, **v5.4.9 ships `_cyrius_init` STB_GLOBAL in `object;` mode (mabda C-launcher unblocked) + sigil 2.8.4**, **v5.4.10 fixes `lib/thread.cyr` post-`clone()` child trampoline + `thread_join` shared/private futex mismatch (majra `cbarrier_arrive_and_wait` unblocked; `tests/tcyr/threads.tcyr` regression coverage)**, self-hosting, IR (40 opcodes, CFG, LASE, DBE), per-arch asm via `#ifdef CYRIUS_ARCH_{X86,AARCH64}` (v5.3.16), multi-width types, sizeof, unions, bitfields, defer (all exit paths), expression-position comparisons, `#assert`, Str/cstr auto-coercion, string interning, syscall arity warnings, `#derive(accessors)`, native multi-return, switch case blocks, `+=`/`-=`/`*=`/`%=`, negative literals, undefined function diagnostic, short-circuit `&&`/`||`, struct initializer syntax, `#regalloc` (multi-register), single-CU DCE, CYML parser. **v5.6.0 opens v5.6.x with the parse.cyr arch-guard cleanup** — fixes 4 real findings from the post-v5.5.40 audit: PARSE_SWITCH jump-table path now gated on x86 (aarch64 falls through to linear-comparison), PARSE_FIELD_LOAD sub-byte movzx hard-errors on aarch64, closure literal `|x| body` address emit gains the same 3-way arch dispatch as `&fn_name`, and the 8 x87/SSE intrinsics (f64_neg/sin/cos/exp/ln/log2/exp2/atan) hard-error on aarch64 instead of silently splicing partial sequences when their helpers stub. cc5 507,136 → 508,880 B (+1,744 B), 19/19 check.sh, byte-identical self-host. **v5.6.1 adds `#else` / `#elif` / `#ifndef` preprocessor directives** backed by a per-level state stack at heap 0x97F10 (64 bytes, 4-state encoding: EMITTING/SEARCHING/DONE/OUTER_SKIP). Closes the long-standing gap where paired `#ifdef` was the only way to branch; the 60+ stdlib paired-`#ifdef` sites can migrate to the cleaner form at leisure. cc5 508,880 → 515,344 B (+6,464 B / +1.27 %), 19/19 check.sh, byte-identical self-host. **v5.6.2 adds explicit overflow operators** — 9 new tokens (`+%` `-%` `*%` wrapping, `+|` `-|` `*|` saturating, `+?` `-?` `*?` checked). Wrapping ops are aliases for bare `+ - *` (wrap on 2's-complement); saturating / checked compile to calls into `lib/overflow.cyr` helpers (`_sat_{add,sub,mul}_i64` / `_chk_{add,sub,mul}_i64`). Checked-panic uses `syscall(60, 57)` — exit code 57 avoids POSIX signal-code and assert-summary conventions. cc5 515,344 → 518,984 B (+3,640 B / +0.71 %), 19/19 check.sh, byte-identical self-host. **v5.6.3 adds `#must_use` + `@unsafe` attributes** backed by a new `fn_flags` table at `0xFC000` (4096 × 8 B). `#must_use` is a fn-level decorator (token 122, same shape as `#regalloc`) — call-site diagnostic fires on bare `ident(args);` statement discard. `@unsafe` is a block-form keyword (token 123) with advisory nested-unsafe warning, no runtime cost. cc5 518,984 → 522,920 B (+3,936 B / +0.76 %), 19/19 check.sh, byte-identical self-host. **v5.6.4 closes the small-language-polish arc with `#deprecated("reason")`** — fn-level attribute (token 124, 11-byte literal) with required string argument. Parsed at PARSE_PROG same shape as `#pe_import`; sets `fn_flags[fi] & 4` + stores string token value in new `fn_deprecated_msg` table at `0x104000`. Call-site diagnostic in PARSE_FNCALL fires at EVERY call site (unlike `#must_use` which only fires on statement-level discard). cc5 522,920 → 525,344 B (+2,424 B / +0.46 %), 19/19 check.sh, byte-identical self-host. **v5.6.5 opens the compiler-optimization arc with Phase O1**: FNV-1a hash for FINDFN (16 KB table at 0x10C000, 8192 slots, load factor 0.5) + `CYRIUS_PROF=1` total-compile-time reporter (clock_gettime wrapper in per-arch fixup.cyr — syscall 228 x86, 113 aarch64) + `docs/development/benchmarks.md` with v5.6.4 → v5.6.5 baseline (409 → 402 ms median self-host, −1.7 %; FINDFN not the hot path, below the 10–25 % prediction). cc5 525,344 → 526,888 B (+1,544 B / +0.29 %), 19/19 check.sh, byte-identical self-host. **v5.6.6 extends CYRIUS_PROF cross-platform**: Windows PE via kernel32!GetTickCount64 (new reroute + `EGETTICKS_PE` emit fn), macOS Mach-O arm64 via libSystem `_clock_gettime_nsec_np` through `__got[6]` (grown from 6 → 7 slots: GOT_SIZE 48→56, BIND_SIZE 72→96, SYMTAB 8→9 entries, STRTAB 80→104, INDIRECT 24→28). Mach-O BSD whitelist extended to include syscall 228. cc5 526,888 → 528,144 B (+1,256 B / +0.24 %), 19/19 check.sh, byte-identical self-host. **v5.6.7 opens Phase O2 peephole** with strength reduction (`x * 2^n → shl`/`lsl`): new `ESHLIMM` / `ESHRIMM` emit helpers (x86 + aarch64) + `_TRY_MUL_BY_POW2` in `PARSE_TERM`. cc5 528,144 → 526,272 B (**−1,872 B** net — peephole applied to cc5 compiling itself). **First optimizer patch that modifies generated bytes**; established the 3-step fixpoint pattern (cc5_a → cc5_b → cc5_c; cc5_b == cc5_c is the new byte-identical check). 19/19 check.sh. **v5.6.8 ships Phase O2 category 2/5**: flag-result reuse + `test rax, rax` replaces ECONDCMP's 10-byte `push/xor/movca/pop/cmp` dance for bare `if (x)` conditions. New `_flags_reflect_rax` tracker in x86 emit helpers (set by arith that updates ZF from rax; cleared by rax-clobbering loads/pops). cc5 526,272 → **504,416 B** (**−21,856 B / −4.15 %** — biggest single-patch optimizer win of v5.6.x so far); self-host compile time 405 → 355 ms (**−12 %**). aarch64 peephole no-ops (non-S-variant arith); wires in for future adds/subs switch. 19/19 check.sh. **v5.6.9 ships Phase O2 category 3/5**: redundant push/pop elim. New `_last_push_cp` tracker in x86 + aarch64 `emit.cyr` — `EPUSHR`/`ESPILL` record `GCP(S)` after emitting the push; `EPOPR`/`EUNSPILL` check `GCP(S) == _last_push_cp` and rewind CP (x86 −1 B, aarch64 −4 B) if nothing was emitted between. 381 adjacent `50 58` pairs in v5.6.8 cc5 → **0**; cc5 504,416 → **504,000 B** (**−416 B** net). Bulk source: v5.6.7's strength-reduction helper (`ESPILL/EUNSPILL` around single-identifier LHS subtrees where nothing needed preserving). aarch64 mirror also fires (cross-built `advanced.tcyr` −488 B, `args.tcyr` −328 B). 19/19 check.sh; 3-step fixpoint b=c=504,000 B. **v5.6.10 ships Phase O2 category 4/5**: commutative combine-shuttle elim. Pre-implementation bytescan found literal `mov + add + add imm → lea` has 0 matches in cc5; retargeted to collapse the 7-byte shuttle `mov rcx,rax; pop rax; op rax,rcx` → 4-byte `pop rcx; op rax,rcx` for commutative ops (ADD/AND/OR/XOR/IMUL). Two-state tracker (`_last_emovca_cp`, `_last_movca_popr_cp`) + `_TRY_COMBINE_SHUTTLE` helper in x86 emit helpers; aarch64 stubs the trackers (no accumulator model). 5861 shuttle sites → **0**; cc5 504,000 → **487,040 B** (**−16,960 B / −3.37 %** — second-largest single-patch win of v5.6.x). Pinned for a later LEA-spirit patch: literal LEA-combine (once IR-aware), SUB as neg+add, CMP flag-flip. 19/19 check.sh; 3-step fixpoint b=c=487,040 B. **v5.6.11 ships Phase O2 category 5/5 — closes Phase O2**: aarch64 combine-shuttle elim. Pre-implementation bytescan found the originally-planned `mul+add→madd` / `and+lsr→ubfx` / signed `sbfx` / `msub` patterns **0×** in cc5_aarch64 (cyrius's combine codegen always shuttles intermediate values through the stack, separating the pair). Retargeted to port v5.6.10's combine-shuttle elim to aarch64 — `src/frontend/parse.cyr`'s combine codegen is SHARED across backends, so v5.6.10's CHANGELOG claim that aarch64 "encodes binary ops with explicit src/dst regs… no accumulator-shuttle counterpart" was wrong. 12-byte `mov x1,x0; ldr x0,[sp],#16; op x0,x0,x1` → 8-byte `ldr x1,[sp],#16; op x0,x0,x1` for commutative ops (ADD 4021 + AND 266 + ORR 95 + MUL 35 + EOR 2 = **4419 sites → 0**; SUB 268 sites intentionally skipped — non-commutative, operand-swap inverts). Native aarch64 cc5: 471,360 → **453,688 B** (**−17,672 B / −3.75 %**); x86 cc5 unchanged at 487,040 B (aarch64-only patch). Wired existing `_last_emovca_cp` / `_last_movca_popr_cp` tracker stubs (v5.6.10-era placeholders) + mirror `_TRY_COMBINE_SHUTTLE` helper in `src/backend/aarch64/emit.cyr`; modified EMOVCA/EPOPR/EADDR/EANDR/EORR/EXORR/EIMUL. Self-host time unchanged (347 ms median — no x86 codegen change). Roadmap reshuffled: originally-planned aarch64 fused ops (`madd`/`msub`/`ubfx`/`sbfx`) re-pinned to **v5.6.18**, post-v5.6.17 linear-scan regalloc (when intermediate values stay in registers, patterns can actually appear adjacent). Cross-platform verification via `ssh pi` / `ssh ecb` / `ssh cass` also surfaced **three pre-existing broad-scope failures** — the NARROW-scope byte-identity invariant (`cc5_a → cc5_b; cc5_a == cc5_b`) still holds on every target, and v5.6.11 output is byte-identical to v5.6.10 for each failure shape. Two distinct root-cause categories: (a) **native-runtime capability gap** — the aarch64 native cc5 can't parse its own source on a Pi (`_TARGET_MACHO` undef, pinned **v5.6.24**), a feature gap in our aarch64 binary's startup path, not a codegen bug; (b) **external platform drift** — our Mach-O and PE bytes are UNCHANGED since v5.5.13 / v5.5.10 (when exit=42 was verified), but macOS Sequoia dyld (pinned **v5.6.25**) and Windows 11 24H2 loader (pinned **v5.6.26**) have tightened their enforcement and now reject our emitted shapes. Our bytes didn't move; the goalposts did. Three stub regression gates added (`tests/regression-aarch64-native-selfhost.sh` / `regression-macho-exit.sh` / `regression-pe-exit.sh`) wired into check.sh — each ships SKIPping with "pin v5.6.2X" message pre-fix, flips to PASS when the slot lands. **22/22 check.sh** including `ssh pi` aarch64 runtime verification plus the 3 skip-stubs. **v5.6.12 opens Phase O3a** (IR-instrument parse emits + enable LASE/DBE + measure floor) — the 590-LOC original single-slot O3 was split into v5.6.12/v5.6.14/v5.6.15/v5.6.16 after recon found 76 parse-site emits (not 50) and 811 already-captured LASE candidates needing a measurement floor before new passes ship. v5.6.12 attempted to enable LASE + DBE and **surfaced a pre-existing correctness bug** — 811 candidates / 5,692 B "savings" turn into 5,692 B of corruption; binary parse-errors on trivial input. Rolled LASE/DBE back to disabled; the precondition (IR_RAW_EMIT markers at 15 sites in parse_*.cyr) IS the load-bearing deliverable. LASE fix pinned to **v5.6.14**. **v5.6.13 = `lib/sha1.cyr` extraction (quick-win)**, pulled forward from v5.6.21 at user request as a confidence-build release between the v5.6.12 LASE discovery and the harder v5.6.14 correctness audit. **v5.6.14 ships the LASE correctness fix** — root-caused the v5.6.12-surfaced corruption to `parse_ctrl.cyr`'s loop constructs missing IR_NOP landing-pad markers at `loop_top = GCP(S)` (4 sites: PARSE_WHILE, for-in, range-for, classic-for). Without the IR_NOP, BB-construction missed the basic-block split at the loop entry, so a LOAD_LOCAL emitted at the start of the loop body was wrongly considered redundant against a STORE_LOCAL outside the loop. Also added `IR_RAX_CLOBBER` (opcode 97) coverage for EMULH/EIDIV/ELODC — three emitters that mutate RAX/RDX without going through MOVR. Re-enabled `ir_apply_lase` under CYRIUS_IR=3; LASE candidates 811 → 564 with cc5 byte-identical and trivial repro PASS. **v5.6.15 ships the IR-emit-order audit fix** — narrowed scope after recon found a pre-existing IR-order vs byte-order inversion in `ESETCC`: the helper recorded `_IR_REC1(S, IR_SETCC, op)` BEFORE calling `ECMPR(S)` (which records IR_CMP). IR stream said SETCC→CMP; byte stream emitted cmp→setcc. 5-LOC fix moves the IR_REC1 call after ECMPR. Verified: SETCC→CMP IR adjacency 3,665 → 0; CMP→SETCC 0 → 3,665. cc5 byte-identical at IR_ENABLED=0 (output bytes unaffected by IR-stream ordering when IR is off). Originally-planned v5.6.15 const-fold + liveness + DCE cascades to v5.6.16; everything +1 through v5.6.29 closeout. Slots total 30 through v5.6.29 closeout.
- **Tests**: 68 .tcyr files, 5 .fcyr fuzz harnesses, 14 .bcyr benchmarks, heap audit, self-hosting (two-step)
