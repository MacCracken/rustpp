...
#
# PINNED: v5.6.23. Ships as a skip-stub pre-fix so check.sh stays
...
#   - CYRIUS_V5623_SHIPPED not set (pre-fix).
set -e
...
if [ -z "$CYRIUS_V5623_SHIPPED" ]; then
    echo "  skip: pin v5.6.23 — Windows 11 24H2 PE syscall(60,42) exits 0x40010080 not 42 (see docs/development/roadmap.md §v5.6.23)"
    exit 0
fi
