...
#
# PINNED: v5.6.22. Ships as a skip-stub pre-fix so check.sh stays
...
#   - CYRIUS_V5622_SHIPPED not set (pre-fix).
set -e
...
if [ -z "$CYRIUS_V5622_SHIPPED" ]; then
    echo "  skip: pin v5.6.22 — macOS arm64 syscall(60,42) exits 1 not 42 (see docs/development/roadmap.md §v5.6.22)"
    exit 0
fi
