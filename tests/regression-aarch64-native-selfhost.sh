...
#   - CYRIUS_V5631_SHIPPED is set (explicit opt-out during v5.6.31 dev).
set -e
...
# Until v5.6.31 ships the fix, this is a known-broken case. Keep
# the stub skipping so CI stays actionable; flip this guard when
# the fix lands.
if [ -z "$CYRIUS_V5631_SHIPPED" ]; then
    echo "  skip: pin v5.6.31 — native aarch64 self-host '_TARGET_MACHO' undef (see docs/development/roadmap.md §v5.6.31)"
    exit 0
fi
