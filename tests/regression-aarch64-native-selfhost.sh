...
#
# PINNED: v5.6.28. Ships as a skip-stub pre-fix so check.sh stays
...
#   - CYRIUS_V5628_SHIPPED is set (explicit opt-out during v5.6.28 dev).
set -e
...
# Until v5.6.28 ships the fix, this is a known-broken case. Keep
# the stub skipping so CI stays actionable; flip this guard when
# the fix lands.
if [ -z "$CYRIUS_V5628_SHIPPED" ]; then
    echo "  skip: pin v5.6.28 — native aarch64 self-host '_TARGET_MACHO' undef (see docs/development/roadmap.md §v5.6.28)"
    exit 0
fi
